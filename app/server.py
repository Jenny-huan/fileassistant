...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        path = urlparse(self.path).path
        if path not in {"/api/docx/inspect", "/api/docx/format"}:
            self._send_json({"error": "Not found"}, HTTPStatus.NOT_FOUND)
            return
...
<body>
  <main>
    <header>
      <div>
        <h1>Word 图片整理助手</h1>
        <p class="sub">上传 .docx，统一图片宽度和对齐方式，处理完成后直接下载新文档。</p>
      </div>
    ...
    <section class="shell">
      <div class="panel work-panel">
        <section>
          ...
          <label class="drop" id="dropZone" for="fileInput">
            <span>
              <strong>点击选择，或把 Word 文档拖到这里</strong>
              <span>支持 .docx，最大 50MB。拖入后会自动分析。</span>
            </span>
          </label>
          <input id="fileInput" type="file" accept=".docx" />
          <div class="file-card">
        ...
        <section>
          ...
          <div id="status" class="status">先选择一个 Word 文档开始。</div>
        </section>
      </div>
  ...
  <script>
    ...
    dropZone.addEventListener("drop", (event) => {
      ...
      if (!file) return;
      if (!file.name.toLowerCase().endsWith(".docx")) {
        setStatus("请拖入 .docx 格式的 Word 文档。", "error");
        return;
      }
      const transfer = new DataTransfer();
      transfer.items.add(file);
    ...
    async function inspectSelectedFile(file) {
      ...
      try {
        ...
        inspectedJobId = payload.job_id;
        renderInspect(payload);
        setStatus("分析完成。确认右侧预览后，点击“整理图片”。", "ready");
      } catch (error) {
        setStatus(error.message, "error");
      } finally {
        inspectBtn.disabled = false;
    ...
    function setSelectedFile(file, autoInspect) {
      ...
      backupLink.hidden = true;
      resetMetrics();
      setStatus(file ? "已选择文档，点击“分析文档”继续。" : "先选择一个 Word 文档开始。", "");
